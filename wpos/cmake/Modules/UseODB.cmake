...
#set(ODB_COMPILE_OUTPUT_DIR "${CMAKE_CURRENT_BINARY_DIR}/${PROJECT_NAME}_odb_autogen")
set(ODB_COMPILE_OUTPUT_DIR "${PROJECT_BINARY_DIR}/odb_autogen")
set(ODB_COMPILE_HEADER_SUFFIX ".hxx")
set(ODB_COMPILE_INLINE_SUFFIX ".i")
set(ODB_COMPILE_SOURCE_SUFFIX ".cpp")
set(ODB_COMPILE_FILE_SUFFIX "-odb")
set(ODB_ARGS -x -fPIC --sql-name-case lower)

set(CMAKE_INCLUDE_CURRENT_DIR TRUE)

function(odb_target_includes outvar)
    foreach(input IN LISTS ARGN)
        get_filename_component(directory "${input}" DIRECTORY)
        set(output_dir ${ODB_COMPILE_OUTPUT_DIR}/${directory})
        list(APPEND ${outvar} ${output_dir})
    endforeach()
    ...
    foreach(input ${PARAM_FILES})
        ...
        foreach(sfx ${ODB_COMPILE_FILE_SUFFIX})
            ...
            if(NOT "${PARAM_MULTI_DATABASE}" MATCHES "static" OR NOT "${pfx}" MATCHES "common")
                set(output "${ODB_COMPILE_OUTPUT_DIR}/${directory}/${fname}${sfx}${ODB_COMPILE_SOURCE_SUFFIX}")
