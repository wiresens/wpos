...
PrinterSamsungSRP350::PrinterSamsungSRP350(const QString& _type,
                                           const QString& _device,
                                           const QString& _user,
                                           const QString& _passwd):
    Printer(_type,_device,_user,_passwd){}
...
void PrinterSamsungSRP350::cmdBold () {
    ...
    put_bytes (PRINTER_DEVICE.toStdString().c_str(), c, 3);
...
void PrinterSamsungSRP350::cmdNormal () {
    ...
    put_bytes (PRINTER_DEVICE.toStdString().c_str(), c, 6);
...
void PrinterSamsungSRP350::cmdCut() {
    ...
    put_bytes (PRINTER_DEVICE.toStdString().c_str(), e, 1);
    put_bytes (PRINTER_DEVICE.toStdString().c_str(), c, 3);
    put_bytes (PRINTER_DEVICE.toStdString().c_str(), e, 1);
}
