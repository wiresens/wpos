...
const uint XmlConfigIO::XML_TAG_IDENTATION  {4};

XmlConfigIO::XmlConfigIO(const QString& file_name,
...
int XmlConfigIO::init(const QString& file_name, QIODevice::OpenMode mode){
...
bool XmlConfigIO::save(const QString& file_name){
...
QString XmlConfigIO::fileName() const{
    return dev.fileName();
}

QString XmlConfigIO::toString() const{
    if (m_domDocument)
...
void XmlConfigIO::debug(const QDomNode& section, uint ident) const{
...
QDomDocument *XmlConfigIO::domDocument()const {
    return m_domDocument;
}

QIODevice::OpenMode XmlConfigIO::openedMode() const{
    return mode;
}

void XmlConfigIO::setXmlHeaderAndFooter(QFile &dev){
    QTextStream stream(&dev);
