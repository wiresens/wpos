    ...
    QWidget(parent)
{
    ...
    if (!pixmapStr.isEmpty())  header->setPixmap(pixmapStr);
    ...
    if (!pixmapStr.isEmpty())  footer->setPixmap(pixmapStr);
...
//@benes    listView.setItemsMovable(false);
    ...
    QFont font;
    if ( !tmpStr.isEmpty() && tmpStr == "SansSerif") font.setFamily("SansSerif");
    ...
    if (!tmpStr.isEmpty())  font.setBold(true);

    tmpStr = xml.readString("underline");
    if (!tmpStr.isEmpty()) font.setUnderline(true);

    tmpStr = xml.readString("italic");
    if (!tmpStr.isEmpty()) font.setItalic(true);

    tmpStr = xml.readString("strikeout");
    if (!tmpStr.isEmpty()) font.setStrikeOut(true);
    ...
    if (!tmpNum.isEmpty()){
        ...
        if (ok) gridWidth = tmpInt ;
    }

    tmpNum = xml.readString("xgridsize");
    if (!tmpNum.isEmpty()){
        tmpInt = tmpNum.toInt(&ok);
        if (ok) gridHeight = tmpInt;
    }
    ...
    if (!spacing.isEmpty()){
        ...
        if (ok) listView.setSpacing(tmpInt);
    }
}
