...
void OfferCreationWidget::typeChangedSlot(const QString& type)
{
    ...
    else if (cpp_operator == "x")
        percent_radiobutton->setChecked(true);
...
void OfferCreationWidget::acceptSlot()
{
    ...
    if (mod.existOffer(xml.toString())) {
        ...
        if (QMessageBox::question(this, tr("Existing offer"), msg, QMessageBox::Yes | QMessageBox::No) == QMessageBox::No)
            return;
    }
    ...
    if (group_box->isChecked()) {
        ...
        if (QMessageBox::question(this, tr("Apply to all", "Apply to all products"), msg, QMessageBox::Yes | QMessageBox::No) == QMessageBox::No)
            return;
...
bool OfferCreationWidget::save()
{
    ...
    if (fixed_radiobutton->isChecked())
        xml.createElement("cpp_operator", "=");
    else if (percent_radiobutton->isChecked())
        xml.createElement("cpp_operator", "x");
