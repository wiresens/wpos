    ...
    , state { (State)_mode }
{
    ...
    if (!user_mod)
        throw NullPtrUserModuleException {};
    if (!db_mod)
        throw NullPtrDatabaseModuleException {};
...
void UserSelectWidget::loadUsers()
{
    ...
    for (auto i = 0; i < xml.howManyTags("user"); i++) {
        ...
        if (i == 0)
            current_item = new QTreeWidgetItem(users_view);
        else
            current_item = new QTreeWidgetItem(users_view, preceding_item);
