...
{
    auto layout = target.layout();
    if(layout) layout->setContentsMargins(margin,margin,margin,margin);
}
    ...
    XmlConfig &xmlDesc, int pos)
{
    ...
    auto tmpStr = xmlDesc.readString( tag + "fontfamily");
    font.setFamily("SansSerif");

    tmpStr = xmlDesc.readString( tag + "fontsize");
    bool ok = false;
    int tmpInt = 0;
    if (!tmpStr.isEmpty()){
    ...
    tmpStr = xmlDesc.readString( tag + "bold");
    font.setBold(tmpStr == "true");

    tmpStr = xmlDesc.readString( tag + "underline");
    font.setUnderline(tmpStr == "true");

    tmpStr = xmlDesc.readString( tag + "italic");
    font.setItalic(tmpStr == "true");

    tmpStr = xmlDesc.readString( tag + "strikeout");
    font.setStrikeOut(tmpStr == "true");
    target.setFont(font);
}
